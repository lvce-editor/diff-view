...
export const handleMessagePort = async (port: any, viewletCommandMap: Readonly<Record<string, unknown>>, setAsRendererProcess = true): Promise<void> => {
  const executeViewletCommand = async (uid: number, command: string, ...args: readonly any[]): Promise<void> => {
    const fn = viewletCommandMap[`DiffView.${command}`]
